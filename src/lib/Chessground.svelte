<script>
  ...
  onMount(async () => {
    ...
    if (disableContextMenu !== undefined)
      config.disableContextMenu = disableContextMenu;
    if (addPieceZIndex !== undefined) config.addPieceZIndex = addPieceZIndex;
    if (addDimensionsCssVarsTo !== undefined)
      config.addDimensionsCssVarsTo = addDimensionsCssVarsTo;
    if (blockTouchScroll !== undefined)
      config.blockTouchScroll = blockTouchScroll;
